...
{
    [Header("対象 outmesh / 検索ルート")]
    public Transform outmeshRoot;
    [Tooltip("対象キー: heart / lung / liver など（body 以外に設定可能）。これを含むオブジェクトを透明操作")]
    ...
    [Header("Preset 連携")]
    public PresetManager presetManager;  // 外部からの自動反映用
    ...
    void HandlePresetAlphaApplied(string keyNormalized, int a255)
    {
        ...
        // UIのみ更新（PresetManagerが描画反映済）
        slider.SetValueWithoutNotify(a255);
        UpdateLabel();
    }

    // ---- α反映（必要なら Transparent 化）
    ...
    void CacheTargets()
    {
        ...
        if (_targets.Count == 0)
            Debug.LogWarning($"[OrganAlphaSlider] '{organKey}' に該当する Renderer が見つかりません。");
    }
