...
/// <summary>
/// スライダーで「body以外」の全体をまとめて濃度(0..255)を一括変更する見本。
/// ・_BaseColor の α を MaterialPropertyBlock で上書き（RGBは維持）
/// ・α<255 のときだけ URP Transparent 判定フラグに書き換え、α=255 で元に戻す
/// ・除外キー（例: "body"）は配列で指定。名前（の一部）が合致するものを無視。
/// ・PresetManager があれば、プリセット適用時にグループの平均をスライダーに反映。
/// </summary>
...
public sealed class GroupAlphaSlider : MonoBehaviour
{
    ...
    [Header("除外キー（例: body）。これらを含まないものを操作")]
    ...
    void ApplyUniformAlpha(int a255)
    {
        ...
        foreach (var (r, subCount) in _targets)
        {
            ...
            for (int i = 0; i < subCount; i++)
            {
                ...
                Color col = (_mpb.GetVector(ID_BaseColor) != Vector4.zero) // MPBに色があれば優先
    ...
    void HandlePresetAlphaApplied(string keyNormalized, int a255)
    {
        ...
        // グループの平均値を算出してスライダーに表示更新のみ（上書きはしない）
    ...
    void CacheTargets()
    {
        ...
        if (_targets.Count == 0)
            Debug.LogWarning("[GroupAlphaSlider] 対象Rendererが見つかりません（全て除外された可能性）。");
    }
    ...
    /// <summary>
    /// GLB 等をランタイムロードした後に、対象 Renderer を再スキャンするためのフック
    /// </summary>
